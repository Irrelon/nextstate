...
var useProps = function useProps(stateControllerMap, ComponentToWrap) {
  ...
  function (_React$PureComponent) {
    _inherits(IrrelonNextStateHOC, _React$PureComponent);
    ...
    function IrrelonNextStateHOC(props) {
      ...
      _this.debugLog("(constructor) *".concat(_this.props.debugTag || _this.constructor.displayName, "* Asking to map state controller data to props..."));

      _this._changeHandlers = _this._changeHandlers || {};
      _this.state = _objectSpread({}, (0, _utils.mapToStateData)(stateControllerMap, _this.props)); // Only hook changes client-side

      if (!process || !process.browser) {
        _this.debugLog("(constructor) *".concat(_this.props.debugTag || _this.constructor.displayName, "* Event listeners not generated because we are running server-side"));

        return _possibleConstructorReturn(_this);
      }

      _this.debugLog("(constructor) *".concat(_this.props.debugTag || _this.constructor.displayName, "* Generating state event listeners..."));

      Object.keys(stateControllerMap).forEach(function (key) {
        var stateController = stateControllerMap[key];
        _this._changeHandlers[key] = _this.generateHandleChangeByKey(_assertThisInitialized(_this), key, stateController);
        stateController.debugLog("(constructor) *".concat(_this.props.debugTag || _this.constructor.displayName, "* Hooking state change event for prop \"").concat(key, "\" in \"").concat(stateController.name(), "\""));
        stateController.on('change', _this._changeHandlers[key]);
      });
      return _this;
    }
    ...
    _createClass(IrrelonNextStateHOC, [{
      ...
      value: function componentWillUnmount() {
        ...
        Object.keys(stateControllerMap).forEach(function (key) {
          var changeHandler = _this2._changeHandlers[key];
          var stateController = stateControllerMap[key];
          stateControllerMap[key].debugLog("(componentWillUnmount) *".concat(_this2.props.debugTag || _this2.constructor.displayName, "* Unhooking state change event for prop \"").concat(key, "\" in \"").concat(stateController.name(), "\""));
      ...
      value: function generateHandleChangeByKey(componentInstance, key, stateController) {
        stateController.debugLog("(generateHandleChangeByKey) *".concat(componentInstance.props.debugTag || componentInstance.constructor.displayName, "* Generating state change event handler for prop \"").concat(key, " in \"").concat(stateController.name(), "\""));
        return function changeHandler() {
          stateController.debugLog("(changeHandler) *".concat(componentInstance.props.debugTag || componentInstance.constructor.displayName, "* Updating prop \"").concat(key, "\" to ").concat(JSON.stringify(stateController.value())));
