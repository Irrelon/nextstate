...
function () {
  ...
  _createClass(StateController, [{
    ...
    value: function debugLog(msg) {
      if (this._debug) {
        console.log("NextState Debug :: ".concat(this.name() || 'Unnamed', " :: ").concat(msg));
      }
    }
  }, {
    key: "debug",
    ...
    value: function update(data) {
      this.debugLog("Asking to update state with ".concat(JSON.stringify(data)));

      if (!Object.is(this._data, data)) {
        this.debugLog("Updating state with ".concat(JSON.stringify(data)));

        if (_typeof(this._data) === 'object' && _typeof(data) === 'object') {
          // Mixin existing data
          this.debugLog("Mixing in ".concat(JSON.stringify(data)));
          this._data = _objectSpread({}, this._data, data);
        } else {
          this.debugLog("Assigning ".concat(JSON.stringify(data)));
          this._data = data;
        }

        this.debugLog("Update completed, new data ".concat(JSON.stringify(this._data)));
      ...
      _createClass(IrrelonNextStateHOC, [{
        ...
        value: function componentWillMount() {
          ...
          Object.keys(stateControllerMap).forEach(function (key) {
            ...
            stateControllerMap[key].debugLog("Hooking state change event for prop \"".concat(key, "\""));
        ...
        value: function componentWillUnmount() {
          ...
          Object.keys(stateControllerMap).forEach(function (key) {
            stateControllerMap[key].debugLog("Unhooking state change event for prop \"".concat(key, "\""));
        ...
        value: function generateHandleChangeByKey(componentInstance, key, stateController) {
          stateController.debugLog("Generating state change event handler for prop \"".concat(key, "\""));
          return function () {
            stateController.debugLog("Updating prop \"".concat(key, "\" to ").concat(JSON.stringify(stateController.value())));
